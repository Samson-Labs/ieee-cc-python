"""Tests for the PPTX extraction Lambda handler."""
...
from src.handlers.pptx_extractor_handler import _parse_event, handler


@pytest.fixture
def mock_extractor():
    with patch("src.handlers.pptx_extractor_handler._extractor") as mock:
...
class TestS3EventInvocation:
    ...
    def test_parses_s3_event(self, mock_extractor):
        event = self._s3_event("my-bucket", "ieee/pending/STD-456.pptx")
        result = handler(event, None)

        assert result["statusCode"] == 200
        mock_extractor.extract.assert_called_once_with(
            bucket="my-bucket",
            key="ieee/pending/STD-456.pptx",
            ou="ieee",
            product_part_number="STD-456",
        )

    def test_invalid_key_pattern_returns_400(self, mock_extractor):
        event = self._s3_event("my-bucket", "random/path/file.pptx")
        result = handler(event, None)
    ...
    def test_s3_event_derives_ou_and_part_number(self):
        params = _parse_event({
            "Records": [{
                "s3": {
                    "bucket": {"name": "b"},
                    "object": {"key": "myou/pending/PART-789.pptx"},
                }
            }]
        })
        ...
        assert params["key"] == "myou/pending/PART-789.pptx"
        ...
        with pytest.raises(ValueError, match="does not match"):
            _parse_event({
                "Records": [{
                    "s3": {
                        "bucket": {"name": "b"},
                        "object": {"key": "no-pending-dir/file.pptx"},
                    }
                }]
            })
