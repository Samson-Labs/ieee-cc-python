#!/usr/bin/env bash
#
# Tear down the Video Transcriber Lambda and associated resources.
...
aws iam delete-role-policy \
    --role-name "${LAMBDA_ROLE_NAME}" \
    --policy-name "S3TranscribeBedrockAccess" 2>/dev/null || true
...
log "Teardown complete. S3 bucket preserved."
