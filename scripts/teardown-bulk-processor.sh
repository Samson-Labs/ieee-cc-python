#!/usr/bin/env bash
#
# Tear down the Bulk Processor Lambda and associated resources.
# S3 bucket and SQS queue are preserved (shared with bulk worker).
#
set -euo pipefail
...
aws iam delete-role-policy \
    --role-name "${LAMBDA_ROLE_NAME}" \
    --policy-name "BulkProcessorAccess" 2>/dev/null || true
